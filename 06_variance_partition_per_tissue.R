...
args <- add_argument(args, '-tissue',
                     help='tissue (abbreviation) to use for variance partition',
                     default='BRNCTX')
args <- add_argument(args, '-log',
                     help='log transform - log(1e-3+x)',
                     default=TRUE)
args <- add_argument(args, '-min_sample',
                     help='In a category variable, a category must have a min number of samples, otherwise set to UNKNOWN for linear regression',
                     default=10)
args <- add_argument(args, '-o',
                     help='output prefix',
                     default="results/gene_BRNCTX")
