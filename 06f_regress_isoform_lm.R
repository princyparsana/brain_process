...
brainExpr <- read.table('20170517.gtex_expression.alltissue.isoform.good_genes.outlier_rm.txt', header=T, sep='\t')
brainCov <- read.table('covariates/20170517.std_covars.PCs.txt', header=T, sep='\t', stringsAsFactors=F, row.names=1)
...
write.table(brainExpr.reg, quote=F, sep='\t', file='20170517.gtex_expression.alltissue.isoform.good_genes.outlier_rm.lm_regressed.txt')
