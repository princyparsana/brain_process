...
FACTOR_NGROUPS <- 6
covars <- read.table('covariates/20170517.all_covariates.PCs.brain.txt', header=T, sep='\t', stringsAsFactors=F)
rownames(covars) <- make.names(covars$st_id)
brainExpr <- read.table('20170517.gtex_expression.brain.good_genes.outlier_rm.txt', header=T)
...
write.table(covarsDropped.mat, file='20170517.all_covar.model_matrix.brain.txt', quote=T, sep='\t')

source('hcp.R')

tissue.hcp <- list()
for ( tissue in sort(unique(covars$tissue_abbrev)) ) {
  samp.idx <- which(covars$tissue_abbrev == tissue)
  ...
  tissue.hcp[[tissue]] <- (t(tis.expr - hcp.res$Z %*% hcp.res$B) * e.sd) + e.mean
  rownames(tissue.hcp[[tissue]]) <- rownames(brainExpr)
  colnames(tissue.hcp[[tissue]]) <- colnames(brainExpr)[samp.idx]
}

tissue.hcp <- do.call(cbind, tissue.hcp)

write.table(tissue.hcp, file='20170517.gtex_expression.brain.good_genes.outlier_rm.HCP.txt', quote=F, sep='\t')
