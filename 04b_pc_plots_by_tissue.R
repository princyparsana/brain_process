...
brainExprFile <- '20170517.gtex_expression.isoform.brain.txt'
...
library(ggplot2)
pdf(sprintf('%s/isoform_covariate_plots.pdf', base_dir))
...
pdf(sprintf('%s/isoform_joint_pcs.pdf', base_dir))
...
for ( tissue in unique(brainCov$tissue_abbrev) ) {
  ...
  pdf(sprintf('%s/isoform.expression.pca.%s.pdf', base_dir, tissue))
...
write.table(samp.values, file='20170517.isoform.sample.outlier.pc.values.txt', quote=F)
write.table(brainExpr[use.genes,], file='20170517.gtex_expression.isoform.brain.good_genes.outlier_rm.txt', quote=F, sep='\t')
