...
args <- add_argument(args, '-hidden',
                     help='hidden factors directory',
                     default='/work-zfs/abattle4/ashis/progdata/brain_process/v6/20170901.gtex_expression.brain.good_genes.outlier_rm.hcp.within_tissue.factors')
...
args <- add_argument(args, '-out_within',
                     ...
                     default="results/lm_hcp_regressd.within.txt")

argv = parse_args(args)
expr_fn <- argv$expr
cov_fn <- argv$cov
hidden_dir <- argv$hidden
...
if(nchar(out_within_fn) > 0){
  ...
  for(tissue in tissues){
    ...
    # add hidden factors
    hidden_factors_fn = sprintf("%s/%s.hidden_factors.txt", hidden_dir, tissue)
    hidden_factors = read_df(hidden_factors_fn)
    rownames(hidden_factors) = make.names(rownames(hidden_factors))
    hidden_predictors = paste0('hidden', 1:ncol(hidden_factors))
    colnames(hidden_factors) = hidden_predictors
    hidden_factors = scale(hidden_factors)
    if(length(intersect(rownames(hidden_factors), rownames(ddf.base))) != nrow(ddf.base))
      stop('samples mismatch in known covariate and hidden factors.')
    hidden_factors = hidden_factors[rownames(ddf.base),]
    ddf.base = cbind(ddf.base, hidden_factors)
    ...
    rhs = paste(c(predictors), collapse = ' + ')
    ...
    rm(my_data, ddf.base, hidden_factors, lm.res)
  }
