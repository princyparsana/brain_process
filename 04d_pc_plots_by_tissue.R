...
# at least one tissue has to have a count above 8 reads

use.genes <- apply(geneMedExpr, 1, max) > 8
...
library(ggplot2)
pdf(sprintf('%s/alltissue_covariate_plots.pdf', base_dir))
...
pdf(sprintf('%s/alltissue_joint_pcs.pdf', base_dir))
...
for ( tissue in unique(allCov$tissue_abbrev) ) {
  ...
  tis.svd <- svd(tis.expr, nu=1, nv=6)
  colnames(tis.svd$v) <- sprintf('%s.PC%d', tissue, 1:6)
  pdf(sprintf('%s/alltissue.expression.pca.%s.pdf', base_dir, tissue))
  ...
  plot(tis.svd$v[,5], tis.svd$v[,6], pch=16, xlab='PC5', ylab='PC5', main=tissue)
  dev.off()
...
write.table(allExpr[use.genes,], file='20170517.gtex_expression.alltissue.good_genes.outlier_rm.txt', quote=F, sep='\t')
